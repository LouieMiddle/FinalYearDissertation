...
The parameters of this approximate distribution are optimized to minimize the Kullback-Leibler (KL) divergence between the approximate and true posterior distributions. 
This optimization is done using a procedure called Variational Inference (VI).
VGP models have several advantages over standard GP models. 
They are more computationally efficient and can handle larger data sets. 
They also allow for more flexible modeling of the covariance structure of the data, as the covariance function can be learned from the data. 
Additionally, VGP models can handle missing data and can be easily extended to multiclass classification problems.

TODO: This has good explanations \citep{Yi2020}
...
"PitchX" and "pitchY" represent where the ball bounced on the pitch. 
...
Whilst it shows that there is clear signs of a good or bad pitch location, one noticeable thing in figures \ref{fig:StumpsXY} and \ref{fig:PitchXY} is how much the data overlaps for different outcomes i.e. locations of zero runs are also locations of six runs.
...
The type of extra is indicated in the "extras" column.
...
I decided not to include extras, but only deliveries where the batter themselves had scored that many runs.
This felt more indicative of a good or a bad delivery, versus a good delivery that the batsmen missed and went for 4 bys.
